...
#'
donor_id <- function(cell_vcf_file, donor_vcf_file = NULL, n_donor=NULL,
    ...
    if (is.null(donor_vcf_file)) {
        vcf_cell <- read_vcf(cell_vcf_file, genome = genome,
                             ...
                             verbose = verbose)
        in_data <- get_snp_matrices(vcf_cell, NULL, verbose = verbose,
                                    donors = donors)
        if (is.null(n_donor))
    ...
    if (model == "Gibbs") {
        if (!is.null(donor_vcf_file))
            stop("If donor genotype is known, we recommand using EM model.")
